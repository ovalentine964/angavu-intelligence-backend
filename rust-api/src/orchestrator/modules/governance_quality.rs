...
impl GovernanceQualityTracker {
    ...
    pub fn default_dimensions() -> Vec<GovernanceDimension> {
        vec![
            GovernanceDimension {
                ...
                weight: 0.20,
                indicators: vec![
                    GovernanceIndicator { name: "Days to register business".into(), value: 23.0, unit: "days".into(), source: "World Bank Doing Business".into(), direction: IndicatorDirection::LowerIsBetter },
                    GovernanceIndicator { name: "Cost (% of income)".into(), value: 28.0, unit: "%".into(), source: "World Bank".into(), direction: IndicatorDirection::LowerIsBetter },
                ],
            },
            GovernanceDimension {
                ...
                weight: 0.20,
                indicators: vec![
                    GovernanceIndicator { name: "Covered market stalls (% of vendors)".into(), value: 30.0, unit: "%".into(), source: "County data".into(), direction: IndicatorDirection::HigherIsBetter },
                    GovernanceIndicator { name: "Road access quality".into(), value: 45.0, unit: "score".into(), source: "KNBS".into(), direction: IndicatorDirection::HigherIsBetter },
                ],
            },
            GovernanceDimension {
                ...
                weight: 0.15,
                indicators: vec![
                    GovernanceIndicator { name: "Bribery incidents per 100 transactions".into(), value: 15.0, unit: "incidents".into(), source: "TI Kenya".into(), direction: IndicatorDirection::LowerIsBetter },
                    GovernanceIndicator { name: "Trust in local government".into(), value: 30.0, unit: "%".into(), source: "Afrobarometer".into(), direction: IndicatorDirection::HigherIsBetter },
                ],
            },
            GovernanceDimension {
                ...
                weight: 0.15,
                indicators: vec![
                    GovernanceIndicator { name: "Informal workers paying tax".into(), value: 12.0, unit: "%".into(), source: "KRA estimates".into(), direction: IndicatorDirection::HigherIsBetter },
                    GovernanceIndicator { name: "Tax-to-service ratio (perceived)".into(), value: 25.0, unit: "score".into(), source: "Survey data".into(), direction: IndicatorDirection::HigherIsBetter },
                ],
            },
            GovernanceDimension {
                ...
                weight: 0.15,
                indicators: vec![
                    GovernanceIndicator { name: "Land title registration rate".into(), value: 35.0, unit: "%".into(), source: "Ministry of Lands".into(), direction: IndicatorDirection::HigherIsBetter },
                    GovernanceIndicator { name: "Eviction protection score".into(), value: 40.0, unit: "score".into(), source: "Legal analysis".into(), direction: IndicatorDirection::HigherIsBetter },
                ],
            },
            GovernanceDimension {
                ...
                weight: 0.15,
                indicators: vec![
                    GovernanceIndicator { name: "Mobile money access".into(), value: 85.0, unit: "%".into(), source: "CBK".into(), direction: IndicatorDirection::HigherIsBetter },
                    GovernanceIndicator { name: "Savings group registration".into(), value: 40.0, unit: "%".into(), source: "SASRA".into(), direction: IndicatorDirection::HigherIsBetter },
                ],
            },
        ]
    }
    ...
    pub fn record(&mut self, index: GovernanceQualityIndex) {
        let entry = self.indices.entry(index.region.clone()).or_insert_with(Vec::new);
        entry.push(index);
        if entry.len() > 100 {
        ...
        for (region, history) in &self.indices {
            if let Some(latest) = history.last() {
                if latest.period == period {
                    let strongest = latest.dimensions.iter()
...
mod tests {
    ...
    fn test_index_computation() {
        ...
        let dims = vec![
            GovernanceDimension {
                name: "Test".into(), name_local: "Test".into(),
                score: 80.0, weight: 1.0, indicators: vec![],
            },
        ];
    ...
    fn test_grading() {
        ...
        for (score, expected_grade) in vec![(90, "A"), (70, "B"), (55, "C"), (40, "D"), (20, "F")] {
            let dims = vec![GovernanceDimension {
                name: "T".into(), name_local: "T".into(),
                score: score as f64, weight: 1.0, indicators: vec![],
            }];
            let index = tracker.compute_index("test", dims, "2026-08");
            assert_eq!(index.grade, expected_grade, "Score {} should grade {}", score, expected_grade);
        }
    }
}
