...
use crate::loops::circuit_breaker::{
    CircuitBreaker as CanonicalCircuitBreaker, CircuitBreakerConfig, FallbackStrategy,
};
...
impl LlmClient {
    ...
    fn pattern_match_fallback(&self, prompt: &str) -> String {
        ...
        if prompt_lower.contains("credit")
            || prompt_lower.contains("score")
            || prompt_lower.contains("risk")
        {
...
impl ModelAgnosticEngine {
    ...
    pub async fn query(&self, request: &providers::InferenceRequest) -> LlmResponse {
        ...
        self.fallback_engine
            .query_deepseek_chat(&request.prompt)
            .await
    }
...
mod tests {
    ...
    async fn test_fallback_pattern_matching() {
        ...
        let response = client
            .query_deepseek_reasoner("Analyze credit score for worker")
            .await;
