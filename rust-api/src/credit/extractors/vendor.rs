...
use crate::credit::types::{MarketTier, TypeFeatures, WorkerType};
...
impl VendorFeatureExtractor {
    ...
    fn product_diversity(&self, transactions: &[Transaction]) -> u8 {
        let products: std::collections::HashSet<String> = transactions
            .iter()
            .filter(|tx| tx.category == TransactionCategory::Sale)
    ...
    fn supplier_count(&self, transactions: &[Transaction]) -> u8 {
        let suppliers: std::collections::HashSet<String> = transactions
            .iter()
    ...
    fn restock_frequency(&self, transactions: &[Transaction]) -> u32 {
        let purchases: Vec<i64> = transactions
            .iter()
            .filter(|tx| tx.category == TransactionCategory::Purchase)
            .map(|tx| tx.timestamp)
            .collect();
    ...
    fn savings_regularity(&self, transactions: &[Transaction]) -> f64 {
        let savings: Vec<i64> = transactions
            .iter()
...
impl WorkerTypeFeatureExtractor for VendorFeatureExtractor {
    fn extract(&self, transactions: &[Transaction], ctx: &WorkerContext) -> TypeFeatures {
        ...
        let features = VendorFeatures {
            ...
            inventory_turnover_days: restock,
            weekend_premium: 1.0,
            savings_regularity: savings_reg,
            restock_frequency_days: restock,
        };

        let fv = vec![
            tier.normalize(),
            (diversity as f64 / 20.0).min(1.0),
            (suppliers as f64 / 10.0).min(1.0),
            (years / 10.0).min(1.0),
            (daily_txn as f64 / 30.0).min(1.0),
            (avg_size / 5000.0).min(1.0),
            (1.0 - (restock as f64 / 14.0).min(1.0)).max(0.0),
            0.5,
            savings_reg,
            0.5,
        ];
