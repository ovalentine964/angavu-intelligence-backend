...
use crate::credit::types::{WorkerType, TypeFeatures, SkillType, AssetValueBucket};
...
impl JuaKaliFeatureExtractor {
    ...
    fn detect_skill(&self, transactions: &[Transaction]) -> SkillType {
        ...
        for tx in transactions {
            if let Some(ref product) = tx.product {
                ...
                if l.contains("weld") || l.contains("metal") || l.contains("chuma") { *skill_scores.entry("welding").or_insert(0) += 1; }
                if l.contains("wood") || l.contains("furniture") || l.contains("mbao") { *skill_scores.entry("carpentry").or_insert(0) += 1; }
                if l.contains("sew") || l.contains("cloth") || l.contains("shona") { *skill_scores.entry("tailoring").or_insert(0) += 1; }
                if l.contains("motor") || l.contains("engine") || l.contains("gari") { *skill_scores.entry("mechanics").or_insert(0) += 1; }
                if l.contains("pipe") || l.contains("plumb") { *skill_scores.entry("plumbing").or_insert(0) += 1; }
                if l.contains("electric") || l.contains("wire") || l.contains("strom") { *skill_scores.entry("electrical").or_insert(0) += 1; }
                if l.contains("cement") || l.contains("brick") || l.contains("tofali") { *skill_scores.entry("masonry").or_insert(0) += 1; }
                if l.contains("paint") || l.contains("rang") { *skill_scores.entry("painting").or_insert(0) += 1; }
            }
        }
    ...
    fn client_repeat_rate(&self, transactions: &[Transaction]) -> f64 {
        let mut client_counts: std::collections::HashMap<String, u32> = std::collections::HashMap::new();
        for tx in transactions {
        ...
        if client_counts.is_empty() { return 0.0; }
    ...
    fn material_cost_ratio(&self, transactions: &[Transaction]) -> f64 {
        let revenue: f64 = transactions.iter().filter(|tx| tx.category == TransactionCategory::Sale).map(|tx| tx.amount).sum();
        let materials: f64 = transactions.iter()
            .filter(|tx| tx.category == TransactionCategory::Purchase)
            .map(|tx| tx.amount).sum();
        if revenue > 0.0 { (materials / revenue).min(1.0) } else { 0.0 }
    }
...
impl WorkerTypeFeatureExtractor for JuaKaliFeatureExtractor {
    fn extract(&self, transactions: &[Transaction], ctx: &WorkerContext) -> TypeFeatures {
        ...
        let avg_project = {
            let sales: Vec<f64> = transactions.iter().filter(|tx| tx.category == TransactionCategory::Sale).map(|tx| tx.amount).collect();
            if sales.is_empty() { 0.0 } else { sales.iter().sum::<f64>() / sales.len() as f64 }
        };
        let cv = {
            let sales: Vec<f64> = transactions.iter().filter(|tx| tx.category == TransactionCategory::Sale).map(|tx| tx.amount).collect();
            if sales.len() < 2 { 0.0 } else {
                let mean = sales.iter().sum::<f64>() / sales.len() as f64;
                if mean < 1.0 { 0.0 } else {
                    let var = sales.iter().map(|s| (s - mean).powi(2)).sum::<f64>() / sales.len() as f64;
                    var.sqrt() / mean
                }
            }
        };
        let locations: u8 = transactions.iter().filter_map(|tx| tx.location.clone()).collect::<std::collections::HashSet<_>>().len() as u8;

        let features = JuaKaliFeatures {
            skill_type: skill, equipment_investment_bucket: AssetValueBucket::Medium,
            client_repeat_rate: repeat, project_completion_signal: 0.7,
            income_irregularity_cv: cv, material_cost_ratio: material_ratio,
            avg_project_value: avg_project, geographic_reach: locations,
            inter_project_savings_rate: savings, years_active: years,
        };

        let fv = vec![
            skill.normalize(), 0.5, repeat, 0.7, 1.0 - cv.min(1.0),
            material_ratio, (avg_project / 50_000.0).min(1.0),
            (locations as f64 / 5.0).min(1.0), savings, (years / 10.0).min(1.0),
        ];

        TypeFeatures::from_features(WorkerType::JuaKaliArtisan, &features, fv, self.feature_names())
    }

    fn worker_type(&self) -> WorkerType { WorkerType::JuaKaliArtisan }
    fn min_transactions(&self) -> usize { 20 }
    fn feature_names(&self) -> Vec<&'static str> {
        vec!["skill_type", "equipment_value", "client_repeat_rate", "completion_signal",
             "income_stability", "material_cost_ratio", "avg_project_value",
             "geographic_reach", "savings_rate", "years_active"]
    }
}
