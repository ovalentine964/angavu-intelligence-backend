...
use crate::credit::types::{AgentTier, TypeFeatures, WorkerType};
...
impl MpesaAgentFeatureExtractor {
    ...
    fn daily_txn_median(&self, transactions: &[Transaction]) -> u32 {
        ...
        for tx in transactions {
            if matches!(
                tx.category,
                TransactionCategory::Sale | TransactionCategory::Commission
            ) {
    ...
    fn detect_tier(&self, daily_txn: u32) -> AgentTier {
        if daily_txn >= 100 {
            AgentTier::SuperAgent
        } else if daily_txn >= 30 {
            AgentTier::Standard
        } else {
            AgentTier::Mini
        }
...
impl WorkerTypeFeatureExtractor for MpesaAgentFeatureExtractor {
    fn extract(&self, transactions: &[Transaction], _ctx: &WorkerContext) -> TypeFeatures {
        ...
        let commission_median = {
            let comms: Vec<f64> = transactions
                .iter()
        ...
        let deposits = transactions
            .iter()
            .filter(|tx| tx.amount > 0.0 && tx.category == TransactionCategory::Sale)
            .count();
        let withdrawals = transactions
            .iter()
            .filter(|tx| tx.amount < 0.0 || tx.category == TransactionCategory::Expense)
            .count();
        let dw_ratio = if withdrawals > 0 {
            deposits as f64 / withdrawals as f64
        } else {
            1.0
        };
        ...
        let features = MpesaAgentFeatures {
            ...
            daily_txn_count_median: daily_txn,
            daily_commission_median: commission_median,
            agent_tier: tier,
            foot_traffic_score: foot_traffic,
            float_efficiency: 0.7,
            deposit_withdrawal_ratio: dw_ratio.min(3.0),
            operating_hours_utilization: foot_traffic,
            revenue_trajectory: 0.0,
            large_txn_frequency: large_freq,
        };

        let fv = vec![
            0.5,
            (daily_txn as f64 / 100.0).min(1.0),
            (commission_median / 5000.0).min(1.0),
            tier.normalize(),
            foot_traffic,
            0.7,
            (dw_ratio / 3.0).min(1.0),
            foot_traffic,
            0.5,
            1.0 - large_freq,
        ];
