...
use crate::credit::types::{WorkerType, TypeFeatures, SkillLevel};
...
impl ConstructionFeatureExtractor {
    ...
    fn detect_skill_level(&self, transactions: &[Transaction]) -> SkillLevel {
        ...
        if wages.is_empty() { return SkillLevel::Helper; }
        let avg = wages.iter().sum::<f64>() / wages.len() as f64;
        if avg > 20_000.0 { SkillLevel::Contractor }
        else if avg > 8_000.0 { SkillLevel::Supervisor }
        else if avg > 3_000.0 { SkillLevel::Fundi }
        else { SkillLevel::Helper }
    }

    fn contractor_count(&self, transactions: &[Transaction]) -> u8 {
        let employers: std::collections::HashSet<String> = transactions.iter()
            .filter(|tx| matches!(tx.category, TransactionCategory::Wage | TransactionCategory::Sale))
    ...
    fn wage_regularity(&self, transactions: &[Transaction]) -> f64 {
        let wages: Vec<i64> = transactions.iter()
            .filter(|tx| tx.category == TransactionCategory::Wage || tx.category == TransactionCategory::Sale)
            .map(|tx| tx.timestamp / 86400)
            .collect();
        if wages.len() < 3 { return 0.0; }
        let mut sorted = wages.clone(); sorted.sort(); sorted.dedup();
            ...
            .filter(|tx| {
                tx.category == TransactionCategory::Expense
                    && tx.product.as_ref().map_or(false, |p| {
                        ...
                        l.contains("tool") || l.contains("jembe") || l.contains("panga")
                            || l.contains("hammer") || l.contains("drill")
                    })
            })
            .map(|tx| tx.amount)
            .sum()
    ...
    fn weekend_work_ratio(&self, transactions: &[Transaction]) -> f64 {
        ...
        let weekend = transactions.iter().filter(|tx| {
...
impl WorkerTypeFeatureExtractor for ConstructionFeatureExtractor {
    fn extract(&self, transactions: &[Transaction], _ctx: &WorkerContext) -> TypeFeatures {
        ...
        let locations: u8 = transactions.iter().filter_map(|tx| tx.location.clone())
            .collect::<std::collections::HashSet<_>>().len() as u8;
        let mpesa_ratio = {
            let total = transactions.len() as f64;
            if total < 1.0 { 0.0 } else {
                transactions.iter().filter(|tx| tx.payment_method == super::PaymentMethod::MPesa).count() as f64 / total
            }
        };

        let features = ConstructionFeatures {
            skill_level: skill, contractor_count: contractors,
            project_frequency_days: 7, wage_regularity: regularity,
            wage_trajectory: 0.0, geographic_mobility: locations,
            tool_investment: tool_inv, weekend_work_ratio: weekend,
            mpesa_payment_ratio: mpesa_ratio, relative_wage_percentile: 0.5,
        };

        let fv = vec![
            skill.normalize(), (contractors as f64 / 5.0).min(1.0),
            0.5, regularity, 0.5, (locations as f64 / 3.0).min(1.0),
            (tool_inv / 50_000.0).min(1.0), weekend, mpesa_ratio, 0.5,
        ];

        TypeFeatures::from_features(WorkerType::ConstructionWorker, &features, fv, self.feature_names())
    }

    fn worker_type(&self) -> WorkerType { WorkerType::ConstructionWorker }
    fn min_transactions(&self) -> usize { 30 }
    fn feature_names(&self) -> Vec<&'static str> {
        vec!["skill_level", "contractor_count", "project_frequency", "wage_regularity",
             "wage_trajectory", "geographic_mobility", "tool_investment",
             "weekend_work", "mpesa_ratio", "wage_percentile"]
    }
}
