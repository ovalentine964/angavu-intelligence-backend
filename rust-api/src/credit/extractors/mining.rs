...
use crate::credit::types::{AssetValueBucket, MineType, MineralType, TypeFeatures, WorkerType};
...
impl MiningFeatureExtractor {
    ...
    fn detect_mineral(&self, transactions: &[Transaction]) -> MineralType {
        for tx in transactions {
            if let Some(ref product) = tx.product {
                ...
                if l.contains("gold") || l.contains("dhahabu") {
                    return MineralType::Gold;
                }
                if l.contains("gem") || l.contains("ruby") || l.contains("sapphire") {
                    return MineralType::Gemstones;
                }
                if l.contains("sand") || l.contains("mchanga") {
                    return MineralType::Sand;
                }
                if l.contains("limestone") || l.contains("cement") {
                    return MineralType::Limestone;
                }
            }
        }
        MineralType::Other
    ...
    fn detect_mine_type(&self, transactions: &[Transaction]) -> MineType {
        let equipment_spend: f64 = transactions
            .iter()
            .filter(|tx| tx.category == TransactionCategory::Expense)
            .map(|tx| tx.amount)
            .sum();
        if equipment_spend > 500_000.0 {
            MineType::Industrial
        } else if equipment_spend > 50_000.0 {
            MineType::SmallScale
        } else {
            MineType::Artisanal
        }
...
impl WorkerTypeFeatureExtractor for MiningFeatureExtractor {
    fn extract(&self, transactions: &[Transaction], _ctx: &WorkerContext) -> TypeFeatures {
        ...
        let features = MiningFeatures {
            ...
            seasonal_income_profile: profile,
            active_period_stability: 0.6,
            buyer_diversity: buyers,
            savings_rate: savings,
            income_trajectory: 0.0,
            avg_inactive_gap_days: gap,
            safety_investment_ratio: safety,
        };

        let fv = vec![
            match mine {
                MineType::Industrial => 1.0,
                MineType::SmallScale => 0.6,
                MineType::Artisanal => 0.3,
            },
            match mineral {
                MineralType::Gold => 1.0,
                MineralType::Gemstones => 0.8,
                MineralType::Sand => 0.4,
                MineralType::Limestone => 0.5,
                MineralType::Other => 0.3,
            },
            0.5,
            0.6,
            (buyers as f64 / 5.0).min(1.0),
            savings,
            0.5,
            (1.0 - (gap as f64 / 60.0).min(1.0)).max(0.0),
            safety,
            0.5,
        ];
