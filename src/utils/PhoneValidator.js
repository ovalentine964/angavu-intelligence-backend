/**
 * PhoneValidator — Kenyan phone number validation for the backend.
 * 
 * Supports:
...
class PhoneValidator {
    ...
    // Carrier prefixes (Kenya)
    static PATTERNS = {
        LOCAL_10_DIGIT: /^0[17]\d{8}$/,           // 07… / 01…
        LOCAL_10_DIGIT_02: /^020\d{7}$/,          // 020…
        BARE_9_DIGIT: /^[17]\d{8}$/,              // 7… / 1…
        INTL_WITH_PLUS: /^\+254[17]\d{8}$/,       // +254…
        INTL_NO_PLUS: /^254[17]\d{8}$/            // 254…
    };
    ...
    static CARRIERS = {
        SAFARICOM: ['0700', '0701', '0702', '0703', '0704', '0705', '0706', '0707', '0708', '0709',
                    ...
                    '0757', '0758', '0759', '0768', '0790', '0791', '0792', '0793', '0794', '0795',
                    '0796', '0797', '0798', '0799'],
     ...
     * @returns {Object} {valid: boolean, normalized: string|null, error: string|null, carrier: string|null}
     */
    static validate(raw) {
        if (!raw || typeof raw !== 'string') {
            return {
                valid: false,
                normalized: null,
                error: 'Namba ya simu haijawekwa.',
                carrier: null
            };
        }
        ...
        if (!/^\+?\d+$/.test(cleaned)) {
            return {
                valid: false,
                normalized: null,
                error: 'Namba ya simu ina herufi zisizotambulika.',
                carrier: null
            };
        }
        ...
        if (!normalized) {
            return {
                valid: false,
                normalized: null,
                error: 'Namba ya simu si sahihi. Tafadhali weka namba ya Kenya (mfano: 0712345678).',
                carrier: null
            };
        }
        ...
        if (!/^\+254[17]\d{8}$/.test(normalized)) {
            return {
                valid: false,
                normalized: null,
                error: 'Namba ya simu si sahihi. Hakikisha unaanzia 07XX au 01XX.',
                carrier: null
            };
        }

        // Detect carrier
        const carrier = this.detectCarrier(normalized);
    ...
    static normalize(cleaned) {
        // +254712345678 → already good
        if (this.PATTERNS.INTL_WITH_PLUS.test(cleaned)) {
            return cleaned;
        }

        // 254712345678 → add +
        if (this.PATTERNS.INTL_NO_PLUS.test(cleaned)) {
            return `+${cleaned}`;
        }

        // 0712345678 / 0112345678 → +254712345678
        if (this.PATTERNS.LOCAL_10_DIGIT.test(cleaned)) {
            return `+254${cleaned.substring(1)}`;
        }

        // 0201234567 → +254201234567 (Telkom landline)
        if (this.PATTERNS.LOCAL_10_DIGIT_02.test(cleaned)) {
            return `+254${cleaned.substring(1)}`;
        }

        // 712345678 → +254712345678 (bare 9-digit)
        if (this.PATTERNS.BARE_9_DIGIT.test(cleaned)) {
            return `+254${cleaned}`;
        }
    ...
    /**
     * Detect carrier from normalized phone number.
     */
    static detectCarrier(normalized) {
        // Get the local part (remove +254 prefix)
    ...
    /**
     * Extract raw digits (no plus, no spaces).
     */
    static toDigits(raw) {
        const result = this.validate(raw);
