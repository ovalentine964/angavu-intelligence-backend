class ReportGenerator {
    async generate({ userId, reportType, date, assistantName, userName, language }) {
        ...
        if (reportType === 'weekly') return this.generateWeeklyReport({ data, assistantName, userName, language });
    ...
    generateDailyReport({ data, assistantName, userName, language }) {
        const templates = {
    ...
    buildDailySwahili(data, assistantName, userName) {
        let report = `📊 *Ripoti ya Leo — ${assistantName}*\n\n👤 ${userName}, hii leo:\n💰 Mauzo: KSh ${this.formatNumber(data.sales)}\n📦 Bidhaa zilizouzwa: ${data.itemsSold}\n📈 Faida: KSh ${this.formatNumber(data.profit)}\n\n`;
        if (data.topProduct) report += `🏆 Bidhaa bora: ${data.topProduct} (KSh ${this.formatNumber(data.topProductSales)})\n`;
        if (data.lowStockItems?.length > 0) report += `⚠️ Stock inayokaribia kuisha: ${data.lowStockItems.join(', ')}\n`;
        report += `\n💡 *Kidokezo: ${data.tip}*\n\n🔗 Pakua Msaidizi: https://github.com/msaidizi/releases\n📤 Shiriki na rafiki: https://msaidizi.app/share`;
        return report;
    }

    buildDailySheng(data, assistantName, userName) {
        let report = `📊 *Report ya Leo — ${assistantName}*\n\n👤 ${userName}, leo:\n💰 Sales: KSh ${this.formatNumber(data.sales)}\n📦 Items zilizouzwa: ${data.itemsSold}\n📈 Profit: KSh ${this.formatNumber(data.profit)}\n\n`;
        if (data.topProduct) report += `🏆 Best seller: ${data.topProduct} (KSh ${this.formatNumber(data.topProductSales)})\n`;
        if (data.lowStockItems?.length > 0) report += `⚠️ Stock ya kuisha: ${data.lowStockItems.join(', ')}\n`;
        report += `\n💡 *Tip: ${data.tip}*\n\n🔗 Download Msaidizi: https://github.com/msaidizi/releases\n📤 Share na boys: https://msaidizi.app/share`;
        return report;
    }

    buildDailyEnglish(data, assistantName, userName) {
        let report = `📊 *Today's Report — ${assistantName}*\n\n👤 ${userName}, today:\n💰 Sales: KSh ${this.formatNumber(data.sales)}\n📦 Items sold: ${data.itemsSold}\n📈 Profit: KSh ${this.formatNumber(data.profit)}\n\n`;
        if (data.topProduct) report += `🏆 Top product: ${data.topProduct} (KSh ${this.formatNumber(data.topProductSales)})\n`;
        if (data.lowStockItems?.length > 0) report += `⚠️ Low stock: ${data.lowStockItems.join(', ')}\n`;
        report += `\n💡 *Tip: ${data.tip}*\n\n🔗 Download Msaidizi: https://github.com/msaidizi/releases\n📤 Share with friends: https://msaidizi.app/share`;
        return report;
    }
    ...
    buildWeeklySwahili(data, assistantName, userName) {
        let report = `📊 *Ripoti ya Wiki — ${assistantName}*\n\n👤 ${userName}, wiki hii:\n💰 Mauzo jumla: KSh ${this.formatNumber(data.sales)}\n📈 Faida jumla: KSh ${this.formatNumber(data.profit)}\n\n`;
        if (data.weeklySales?.length > 0) {
            const sorted = [...data.weeklySales].sort((a, b) => b.amount - a.amount);
            report += `📊 Mauzo ya juu: ${sorted[0].day} (KSh ${this.formatNumber(sorted[0].amount)})\n📉 Mauzo ya chini: ${sorted[sorted.length - 1].day} (KSh ${this.formatNumber(sorted[sorted.length - 1].amount)})\n\n📅 Mauzo ya kila siku:\n`;
            for (const day of data.weeklySales) { report += `  ${day.day}: ${'█'.repeat(Math.floor(day.amount / 1000))} KSh ${this.formatNumber(day.amount)}\n`; }
        }
        report += `\n💡 *Kidokezo: ${data.tip}*\n\n🔗 Pakua Msaidizi: https://github.com/msaidizi/releases`;
        return report;
    }
    ...
    buildWeeklySheng(data, assistantName, userName) {
        let report = `📊 *Report ya Wiki — ${assistantName}*\n\n👤 ${userName}, wiki hii:\n💰 Total sales: KSh ${this.formatNumber(data.sales)}\n📈 Total profit: KSh ${this.formatNumber(data.profit)}\n\n`;
        if (data.weeklySales?.length > 0) {
            const sorted = [...data.weeklySales].sort((a, b) => b.amount - a.amount);
            report += `📊 Best day: ${sorted[0].day} (KSh ${this.formatNumber(sorted[0].amount)})\n📉 Worst day: ${sorted[sorted.length - 1].day} (KSh ${this.formatNumber(sorted[sorted.length - 1].amount)})\n\n📅 Daily breakdown:\n`;
            for (const day of data.weeklySales) { report += `  ${day.day}: ${'█'.repeat(Math.floor(day.amount / 1000))} KSh ${this.formatNumber(day.amount)}\n`; }
        }
        report += `\n💡 *Tip: ${data.tip}*\n\n🔗 Download Msaidizi: https://github.com/msaidizi/releases`;
        return report;
    ...
    getRandomTip(language) {
        const tips = {
            sw: ['Mandazi yanaongezeka soko leo. Ongeza stock!', 'Jumatatu ndio soko yako bora. Fungua mapema!', 'Wateja wanaanza kuja saa 10. Hakikisha uko tayari!', 'Ongeza bei ya bidhaa zinazouzwa sana — mahitaji yapo!', 'Fanya hesabu za mwisho wa siku kabla ya kufunga.', 'Wateja wanaopenda huduma nzuri hurudi tena. Tabasamu!', 'Sukari inauzwa sana wiki hii. Ongeza order!', 'Punguza bei ya bidhaa zinazokaa sana — cash flow ni muhimu!'],
            sheng: ['Mandazi zinapiga soko leo. Increase stock! 💪', 'Monday ndio best day yako. Open mapema!', 'Customers wanaanza kuja saa 10. Kuwa ready!', 'Ongeza bei ya items zinazouzwa — demand iko!', 'Fanya hesabu za evening kabla ya kufunga.', 'Customers wanaopenda poa service hurudi. Smile!', 'Sukari inauzwa sana wiki hii. Order more!', 'Punguza bei ya slow movers — cash flow ni king!'],
            en: ['Mandazi are selling well today. Stock up!', 'Monday is your best sales day. Open early!', 'Customers start coming at 10 AM. Be ready!', 'Raise prices on fast-selling items — demand is there!', 'Do your end-of-day calculations before closing.', 'Customers who get good service come back. Smile!', 'Sugar is selling fast this week. Order more!', 'Discount slow-moving items — cash flow is king!']
        };
