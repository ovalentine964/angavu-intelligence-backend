...
class Settings(BaseSettings):
    ...
    # === Encryption ===
    ENCRYPTION_KEY: str = "change-me-32-byte-key-for-aes-256"
    ...
    OPENWA_WEBHOOK_SECRET: str = "change-me"
    GROQ_API_KEY: str = ""
    ...
    def validate_webhook_secret(cls, v, info):
        env = os.getenv("APP_ENV", info.data.get("APP_ENV", "development"))
        if env == "production":
            if not v or v == "change-me" or len(v) < 16:
                raise ValueError("OPENWA_WEBHOOK_SECRET must be set and at least 16 characters in production")
            return v
        if v == "change-me":
